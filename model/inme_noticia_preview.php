...
/**
 * Description of noticia_preview
 *
 * @author carlos
 */
class inme_noticia_preview
{
        ...
        foreach ($links as $link) {
            if (mb_substr($link, 0, 19) == 'http://i.imgur.com/') {
                ...
                if (count($parts) >= 4) {
                    ...
                    if (is_numeric($video_id)) {
                        if (!file_exists($this->filename)) {
                            if (self::$downloads > 0) {
                                try {
                                    $html = fs_file_get_contents('http://vimeo.com/api/v2/video/' . $video_id . '.php');
                                    if ($html) {
                                        $hash = unserialize($html);
                                        if (isset($hash[0]['thumbnail_medium'])) {
                                            fs_file_download($hash[0]['thumbnail_medium'], 'tmp/vimeo/' . $video_id);
                                            $this->type = 'vimeo';
                ...
                if (!file_exists($filename)) {
                    if (self::$downloads > 0) {
                        $html = fs_file_get_contents($link);
                        $urls = array();
